...
public class Sender {
    ...
    public void senderThread() {
        ...
        if (gmsSendListInDb.isEmpty()) return; // 비어있다면 리턴
        ...
        if (unsentGmsSendList.isEmpty()) return;

        // 전송 요청 포맷 (비정상 요청 건 - DB에서 읽어왔지만 무슨 이유로 전송되지 않은 데이터들)
        dataForRequest = gmsSendService.changeDataFormatForRequestSendMessage(unsentGmsSendList);
